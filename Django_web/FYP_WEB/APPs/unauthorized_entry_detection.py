...
class Unauthorized(object):
    ...
    def getFrame(self):
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, facesCurFrame):
                ...
                for keys, values in self.load_images.dictionary.items():
                    if matched_image in values:
                        ...
                        if (splitted[1] == 'CUSTOMER'):
                            ...
                            cv2.putText(self.frame, f'Unauthorized, Customer ALERT', (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                                        (0, 255, 0), 3, 1)


                        if (splitted[1] == 'OFFENDER'):
                            bounding_boxes(splitted[0], 'Known Offender', (0, 0, 255))
                            cv2.putText(self.frame, f'Unauthorized, Offender ALERT', (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                                        (0, 0, 255), 3, 1)
            ...
            if name == 'unknown':
                ...
                winsound.Beep(500, 250)
                cv2.putText(self.frame, f'Unauthorized, Unknown Person ALERT', (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                            (0, 0, 255), 3, 1)
    ...
    def UnauthorizedEntry(self):
        """authorization except employees are then registered an warning"""

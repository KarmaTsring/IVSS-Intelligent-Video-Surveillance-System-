...
class Exit(object):
    ...
    def connection(self, name, splitted):
        ...
        if Exits.objects.filter(personNames=name).exists():
            pass
        else:
            data_entry = Exits(personNames=name, person_post= splitted, personExitTime=dtString)
            data_entry.save()
            ...
            if matches[matchIndex]:
                for keys, values in self.load_images.dictionary.items():
                    if matched_image in values:
                        ...
                        self.connection(splitted[0], splitted[1])
            if name == 'unknown':
